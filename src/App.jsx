    ...
    <div className={`min-h-screen w-full transition-colors duration-300 ${darkMode ? 'dark-mode' : ''} ${
            ...
            {showSettings && (
              <div className="absolute right-0 mt-2 w-56 bg-white rounded-2xl shadow-2xl border border-gray-200 z-[9999]">
                ...
                <button
                  ...
                  className="w-full px-4 py-3 text-left text-red-400 hover:bg-gray-100 hover:text-red-300 rounded-b-lg flex items-center gap-3"
                >
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                  </svg>
      ...
      <div className="max-w-7xl mx-auto px-6 py-10">
          ...
          <div>
            ...
            <div className="mb-10">
              <div className="bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-2xl p-4 mb-6 text-center">
                <h2 className="text-2xl font-bold mb-2">📊 GRÁFICOS INTERATIVOS</h2>
                <p className="text-sm opacity-90">Adiciona transações e contas para ver os gráficos populados!</p>
              </div>
              
              <div className="grid grid-cols-3 gap-8">
              {/* Gráfico 1: Evolução do Patrimônio */}
              <div className={`rounded-3xl border p-8 ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                ...
                <ResponsiveContainer width="100%" height={250}>
                  <AreaChart data={(() => {
                    ...
                    const useExample = totalWealth === 0;
                    
                    for (let i = 5; i >= 0; i--) {
                      const date = new Date(today.getFullYear(), today.getMonth() - i, 1);
                      const month = date.toLocaleDateString('pt-PT', { month: 'short' });
                      const value = useExample ? Math.random() * 5000 + 1000 : totalWealth;
                      data.push({ month, valor: value });
                    }
                    return data;
                  })()}>
                    <defs>
                    ...
                    <Tooltip 
                      ...
                      formatter={(value) => ['€' + value.toFixed(2), 'Patrimônio']}
                    />
                    <Area 
                      type="monotone" 
                      dataKey="valor" 
                  ...
                  <PieChart>
                    <Pie
                      data={(() => {
                        ...
                        if (!hasData) {
                          expensesByCategory['Alimentação'] = 450;
                          expensesByCategory['Transporte'] = 200;
                          expensesByCategory['Lazer'] = 150;
                          expensesByCategory['Saúde'] = 100;
                        }
                        
                        const colors = ['#ef4444', '#f97316', '#eab308', '#22c55e', '#3b82f6', '#a855f7', '#ec4899', '#06b6d4'];
                        return Object.keys(expensesByCategory).map((key, i) => ({
                          name: key,
                ...
                <ResponsiveContainer width="100%" height={250}>
                  <BarChart data={(() => {
                    ...
                    for (let i = 5; i >= 0; i--) {
                      ...
                      if (monthIncome > 0 || monthExpense > 0) hasData = true;
                      
                      // Se não há dados, usar exemplo
                      const income = hasData ? monthIncome : Math.random() * 2000 + 1000;
                      const expense = hasData ? monthExpense : Math.random() * 1500 + 500;
                      
                      data.push({ month, receitas: income, despesas: expense });
                    }
                    return data;
                  })()}>
                    <CartesianGrid strokeDasharray="3 3" stroke={darkMode ? '#374151' : '#e5e7eb'} />
        ...
        {activeTab === 'transactions' && (
          <div>
            ...
            <div className="bg-white rounded-3xl border border-gray-200 p-8">
              {transactions.length === 0 ? (
                <p className="text-gray-500 text-center py-8">Ainda não tens transações</p>
              ) : (
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl max-w-md w-full p-8 border border-gray-200">
            ...
            {modalType === 'transaction' && (
              <div className="space-y-6">
                ...
                <input type="number" step="0.01" placeholder="Valor (€)" value={formData.amount || ''} onChange={(e) => setFormData({...formData, amount: e.target.value})} className="w-full px-4 py-3 bg-gray-50 border border-gray-300 rounded-2xl text-gray-800 placeholder-gray-400" />
                <input type="text" placeholder="Descrição" value={formData.description || ''} onChange={(e) => setFormData({...formData, description: e.target.value})} className="w-full px-4 py-3 bg-gray-50 border border-gray-300 rounded-2xl text-gray-800 placeholder-gray-400" />
                <div>
                  <label className="text-gray-500 text-sm mb-2 block">Data da Transação</label>
                  <input type="date" value={formData.date || ''} onChange={(e) => setFormData({...formData, date: e.target.value})} className="w-full px-4 py-3 bg-gray-50 border border-gray-300 rounded-2xl text-gray-800" />
                </div>
            ...
            <div className="flex gap-4 mt-6">
              <button onClick={handleSubmit} className="flex-1 bg-green-600 hover:bg-green-700 text-gray-800 px-4 py-3 rounded-full font-semibold">Adicionar</button>
              <button onClick={closeModal} className="flex-1 bg-gray-50 hover:bg-gray-100 text-gray-800 px-4 py-3 rounded-full font-semibold">Cancelar</button>
            </div>
          </div>
        </div>
      )}
